//#region node_modules/.pnpm/@lovable.dev+cloud-auth-js@1.1.2/node_modules/@lovable.dev/cloud-auth-js/dist/index.js
var package_default = {
  ...
  keywords: ["lovable", "oauth", "authentication"],
  license: "MIT",
  devDependencies: {
    "@types/node": "^22",
    jsdom: "^26",
...
function getPopupDimensions(isInIframe) {
  ...
  return {
    width,
    height,
    left: hasBrowserPosition
      ? window.screenX + (window.outerWidth - width) / 2
      : (window.screen.width - width) / 2,
    top: hasBrowserPosition
...
function processOAuthResponse(data, expectedState) {
  ...
  if (!data.access_token || !data.refresh_token)
    return { error: /* @__PURE__ */ new Error("No tokens received") };
  return {
    tokens: {
...
function createAuth(config = {}) {
  ...
  async function signInWithOAuth(provider, opts = {}) {
    ...
    const { messagePromise, cleanup } = startWebMessageListener(
      isDesktopApp ? [...supportedOAuthOrigins, window.location.origin] : supportedOAuthOrigins,
    );
    let popup;
    ...
    if (!popup && (isMobileApp || isDesktopApp)) {
      ...
      try {
        return processOAuthResponse(
          await Promise.race([messagePromise, webViewTimeoutPromise]),
          state,
        );
      } catch (error) {
