...
- [ ] Fix checkout so cards are accepted — CONFIG (no code): in host env set STRIPE_SECRET_KEY=sk_live_…, STRIPE_LIFETIME_PRICE_ID=<live price id>, STRIPE_WEBHOOK_SECRET=<live whsec_…> after Stripe KYC. Code is now env-ready (products.ts).
